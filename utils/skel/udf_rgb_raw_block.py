...
def _write_rgb_raw(inarray, outarray, bandnames, nodata):
    ...
    with warnings.catch_warnings():
        ...
        red_median = np.nanmedian(rgbn[:, 0, :, :], axis=0)
        green_median = np.nanmedian(rgbn[:, 1, :, :], axis=0)
        blue_median = np.nanmedian(rgbn[:, 2, :, :], axis=0)
        nir_median = np.nanmedian(rgbn[:, 3, :, :], axis=0)
    ...
    outarray[3][valid_pixels] = np.rint(nir_median[valid_pixels])
...
def forcepy_chunk(inarray, outarray, dates, sensors, bandnames, nodata, nproc):
    _write_rgb_raw(inarray, outarray, bandnames, nodata)
